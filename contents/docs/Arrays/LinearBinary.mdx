...
You must already be familiar with these two methods we have for searching in an array, linear and binary search. We had analyzed them and got the result that for a sorted array, the fastest method to search is the binary one. Today, we’ll learn how to code them and practically use them to search. 
